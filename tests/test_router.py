...
async def test_router_fails_when_jina_and_cloakbrowser_return_block_pages(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from scrapefold.router import walk

    async def _fetch_block(self: ScrapeEngine, url: str, opts: ScrapeOptions) -> ScrapeResult:
        text = (
            "Warning: Target URL returned error 403" + "x" * 300
            if self.NAME == "jina"
            else "403 ERROR / The request could not be satisfied / Generated by cloudfront"
            + "x" * 500
        )
        return ScrapeResult(
            url=url,
            ...
            engine=self.NAME,
            elapsed_ms=1,
            meta={"status_code": 200} if self.NAME == "jina" else {},
        )

    for name in ("jina", "cloakbrowser"):
        cls = type(
            f"_Blocked_{name}",
            (ScrapeEngine,),
            {
    ...
    with pytest.raises(AllEnginesFailed) as exc_info:
        await walk("https://example.com/", ScrapeOptions(engines=("jina", "cloakbrowser")))

    assert exc_info.value.failures == ["jina:suspicious", "cloakbrowser:suspicious"]
