...
class TestIsSuspiciousAntibotPhrases:
    @pytest.mark.parametrize(
        ("text", "meta"),
        [
            ("Warning: Target URL returned error 403" + "x" * 300, {"status_code": 200}),
            (
                "403 ERROR / The request could not be satisfied / Generated by cloudfront"
                + "x" * 500,
                {},
            ),
        ],
    ...
    def test_wrapped_block_pages_are_suspicious(self, text: str, meta: dict) -> None:
        assert is_suspicious(_result(text=text, meta=meta)) is True
